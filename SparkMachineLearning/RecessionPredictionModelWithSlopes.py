...
parsedData = training.map(lambda line: LabeledPoint([float(x) for x in line.split(',')[1:8]][6],
...
#Test
parsedData1 = test.map(lambda line: LabeledPoint([float(x) for x in line.split(',')[1:8]][6],
                                                 array([float(x) for x in line.split(',')[1:8]])))
