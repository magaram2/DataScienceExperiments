...
parsedData = training.map(lambda line: LabeledPoint([float(x) for x in line.split(',')][4],
...
#Test
parsedData1 = test.map(lambda line: LabeledPoint([float(x) for x in line.split(',')][4], array([float(x) for x in line.split(',')])))
